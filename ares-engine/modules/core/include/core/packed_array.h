...
namespace ares::core {
	...
	template <typename value, unsigned int array_size, typename allocator = default_allocator>
	class packed_array
	{
	public:
		friend class packed_array_iterator<value, array_size, allocator>;
		...
		using iterator = packed_array_iterator<value_type, array_size, allocator>;
		using const_iterator = packed_array_iterator<const value_type, array_size, allocator>;
		using reverse_iterator = eastl::reverse_iterator<iterator>;
		using const_reverse_iterator = eastl::reverse_iterator<const_iterator>;
		using allocator_type = sys_allocator<allocator>;

		packed_array();
		packed_array(allocator_type& alloc);
	...
	protected:
		pointer get_offset(size_type index);
	...
	template <typename value, unsigned int array_size, typename allocator>
	packed_array<value, array_size, allocator>::packed_array()
		: count_(array_size), allocator_()
	{
		initialize();
	}

	template <typename value, unsigned int array_size, typename allocator>
	packed_array<value, array_size, allocator>::packed_array(allocator_type& alloc)
		: count_(array_size), allocator_(alloc)
	{
		initialize();
	}

	template <typename value, unsigned int array_size, typename allocator>
	packed_array<value, array_size, allocator>::packed_array(const packed_array& other)
	...
	template <typename value, unsigned int array_size, typename allocator>
	packed_array<value, array_size, allocator>& packed_array<value, array_size, allocator>::operator=(const packed_array& other)
	{
		if (this != &other)
		{
	...
	template <typename value, unsigned int array_size, typename allocator>
	packed_array<value, array_size, allocator>::packed_array(packed_array&& other) noexcept
		: buffer_(eastl::exchange(other.buffer_, nullptr)), count_(eastl::move(other.count_)), allocator_(eastl::move(other.allocator_))
	{
	}

	template <typename value, unsigned int array_size, typename allocator>
	packed_array<value, array_size, allocator>& packed_array<value, array_size, allocator>::operator=(packed_array&& other) noexcept
	{
		if (this != &other)
		{
	...
	template <typename value, unsigned int array_size, typename allocator>
	typename packed_array<value, array_size, allocator>::reference packed_array<value, array_size, allocator>::at(size_type index)
	{
	...
	template <typename value, unsigned int array_size, typename allocator>
	typename packed_array<value, array_size, allocator>::const_reference packed_array<value, array_size, allocator>::at(size_type index) const
	{
		if (index >= count_) throw std::out_of_range("Index out of range.");
		return *get_offset(index);
	}
	...
	template <typename value, unsigned int array_size, typename allocator>
	void packed_array<value, array_size, allocator>::swap(packed_array& other) noexcept
	{
	...
	template <typename value, unsigned int array_size, typename allocator>
	typename packed_array<value, array_size, allocator>::pointer packed_array<value, array_size, allocator>::get_offset(size_type index)
	{
	...
	template <typename value, unsigned int array_size, typename allocator>
	typename packed_array<value, array_size, allocator>::size_type packed_array<value, array_size, allocator>::get_buffer_size() const
	{
	...
	template <typename value, unsigned int array_size, typename allocator>
	void packed_array<value, array_size, allocator>::initialize()
	{
